...
import { PageDiagnostic, GscDataRow } from '../../../types';
...
export async function POST(req: Request) {
  try {
    ...
    const { pageUrl, pageDiagnostics, gscData } = body as { pageUrl: string; pageDiagnostics: PageDiagnostic[], gscData?: GscDataRow[] };

    if (!pageUrl || !pageDiagnostics) {
      return Response.json({ error: 'pageUrl and pageDiagnostics are required.' }, { status: 400 });
    }

    const report = await deepAnalysisFlow({ pageUrl, pageDiagnostics, gscData });
